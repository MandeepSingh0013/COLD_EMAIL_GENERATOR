    ...
    def process_submission(self):
        try:
            ...
            for job in jobs:
                ...
                st.session_state.email = self.chain.write_mail_with_translation(self.model_choice, job, links, self.selected_language)
...
#     with col1:
#         st.subheader("Select LLM Model")
#         model_choice = st.radio("Select the LLM model:",("LLama", "Gemma", "Mixtral"),
#         index=0
#     )
...
#         # disabled=st.session_state.language_translate
#     )
    
#     with col3:
#         st.subheader("Upload Protfolio CSV")
#         st.write("Please upload a CSV with the following structure:")
...
#         if uploaded_file:
#                 try:
#                     st.session_state.status=portfolio.load_csv(uploaded_file)
#                     if st.session_state.status == "success":
#                         st.success("CSV Uploaded and Validated Successfully!")
#                     else:
...
#     # Input for job URL
#     url_input = st.text_input("Enter a Job URL", placeholder="https://jobs.nike.com/job/R-37070?from=job%20search%20funnel")
...
#                     st.session_state.email = llm.write_mail_with_translation(model_choice,job,links,selected_language)
#                     st.subheader(f"Email Generated in ({selected_language}) by ({model_choice}) model")
#                     st.code(st.session_state.email,language="markdown")
